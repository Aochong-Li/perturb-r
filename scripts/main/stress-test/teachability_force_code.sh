...
# Experiment B: Code leakage detection via forced generation
# Tests whether teacher steers for HumanEval/MBPP already contain enough
# information to solve the problem without additional reasoning.
#
# For each teacher_guide pickle:
#   1. Filter to coding benchmarks (humaneval, mbpp, mbppplus)
#   2. Construct prompts with forced </think> after the steer
#   3. Generate code via vLLM
#   4. Execute against test cases and record pass/fail
...
OUTPUT_DIR="./results/${DATASET_NAME}/teacher_guide/teachability_force_code"
DATASET_PATH="./data/allcode"

# Models that have existing coding teacher_guide results
...
echo "$MODELS_INFO" | while IFS=, read -r model_name nick_name; do
    echo "=== Teachability Force Code: $nick_name (model: $model_name) ==="
