...
from reward_score.math_eval import math_if_boxed, last_boxed_only_string, remove_boxed
...
class TeacherGuide(OpenLMEngine):
    ...
    def load_dataset(self) -> None:
        ...
        stats = self.df.groupby(["problem", "source"])[["model_is_correct"]].sum().reset_index().rename(columns = {"model_is_correct": "solve_n"})
        ...
        self.df = self.df.merge(stats, on = ["problem", "source"]).reset_index(drop=True).rename(columns = {"solve_n": "student_solve_n"})
